...
//
// This code is based on https://github.com/WebFreak001/code-debug/blob/f204573fae2f9614aba4be3307984c28576f3b8d/src/backend/gdb_expansion.ts
...
export function isExpandable(value: string): number {
  ...
  if (value.length === 0) {
    ...
    return 0;
  } else if (nullpointerRegex.exec(value)) {
    return 0;
  } else if (referenceStringRegex.exec(value)) {
    return 0;
  } else if (referenceRegex.exec(value)) {
    return 2; // reference
  } else if (charRegex.exec(value)) {
    return 0;
  } else if (numberRegex.exec(value)) {
    return 0;
  } else if (variableRegex.exec(value)) {
    return 0;
  } else if (errorRegex.exec(value)) {
    return 0;
  } else {
    return 0;
  }
...
export function expandValue(
  variableCreate: (arg: VariableObject | string, options?: any) => any,
  value: string,
  root: string = '',
  extra: any = undefined,
): any {
  ...
  const stack = [root];
  let parseValue: () => any,
    parseCommaResult: (pushToStack: boolean) => any,
    parseCommaValue: () => any,
    parseResult: (pushToStack: boolean) => any,
    createValue: (name: string, val: any) => any;
  let variable = '';
