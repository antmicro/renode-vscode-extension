...
//
// This code is based on https://github.com/WebFreak001/code-debug/blob/f204573fae2f9614aba4be3307984c28576f3b8d/src/mibase.ts
...
export class MI2DebugSession extends DebugSession {
  ...
  protected useVarObjects = false;
  protected quit = false;
  protected attached = false;
  protected initialRunCommand = RunCommand.NONE;
  protected stopAtEntry: boolean | string = false;
  protected started = false;
  protected crashed = false;
  protected miDebugger: MI2 | undefined;
        ...
        Promise.all(all).then(
          brkpoints => {
            ...
            brkpoints.forEach(brkp => {
              ...
              if (brkp[0]) {
                finalBrks.push(
                  new DebugAdapter.Breakpoint(true, brkp[1]!.line),
                );
              }
            });
            response.body = {
    ...
    this.miDebugger!.getStack(
      ...
      stack => {
        ...
        stack.forEach(element => {
          ...
          frame.instructionPointerReference = element.address;
          ret.push(frame);
        });
        response.body = {
          stackFrames: ret,
    ...
    const createScope = (scopeName: string, expensive: boolean): Scope => {
      ...
      if (this.scopeHandlesReverse.hasOwnProperty(key)) {
        ...
        handle = this.variableHandles.create(
          new VariableScope(scopeName, threadId, level),
        );
